...
!!! warning "Custom resolv.conf required"
    Oracle VMs have a VCN search domain appended to `/etc/resolv.conf`. This causes transient DNS failures inside pods after node restarts.
...
All nodes are connected via Tailscale. Node IPs on the Tailscale interface (`tailscale0`) are used for k3s inter-node communication, configured in `ansible/inventory/group_vars/k3s_cluster.sops.yml`:
